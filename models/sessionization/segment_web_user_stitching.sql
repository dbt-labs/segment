with source as (
    ...
    where email is not null and (user_id is not null or anonymous_id is not null)

)

, identify as (

--Each email can be associated with only 1 user, however a user can have many emails
...
--Each email can be associated with only 1 id, however an id can have many emails
